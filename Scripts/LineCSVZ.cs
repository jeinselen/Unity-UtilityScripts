...
[ExecuteInEditMode]
public class LineCSVZ : MonoBehaviour
{
	...
	private void ParseCSV(string fileCont)
	{
		...
		foreach (var line in lines)
		{
			...
			float x = float.TryParse(parts[0], out x) ? x*Scale.x+Offset.x : 0.0f;
			float y = 0.0f;
			float z = float.TryParse(parts[1], out z) ? z*Scale.z+Offset.z : 0.0f;
			if (parts.Length >= 3) {
				y = float.TryParse(parts[1], out y) ? y*Scale.y+Offset.y : 0.0f;
				z = float.TryParse(parts[2], out z) ? z*Scale.z+Offset.z : 0.0f;
			}
